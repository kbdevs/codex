...
impl CodexThread {
    ...
    /// Rolls back the latest persisted turn without creating a client request.
    pub async fn rollback_latest_turn(&self) -> Result<(), String> {
        crate::session::rollback_thread(&self.session, uuid::Uuid::new_v4().to_string(), 1).await
    }
