...
impl App {
    ...
    pub(crate) fn undo_last_turn(&mut self) {
        ...
        let previous_base_id = self.backtrack.base_id;
        self.backtrack.base_id = Some(thread_id);
        let selection = self.backtrack_selection(nth_user_message);
        self.backtrack.base_id = previous_base_id;
        if let Some(selection) = selection {
            self.apply_backtrack_selection(selection);
        } else {
            self.chat_widget
                .add_error_message("The latest turn cannot be undone.".to_string());
        }
    }
