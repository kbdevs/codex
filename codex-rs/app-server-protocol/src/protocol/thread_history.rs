...
impl ThreadHistoryBuilder {
    ...
    fn handle_thread_rollback(&mut self, payload: &ThreadRolledBackEvent) {
        ...
        let user_turn_positions = self
            ...
            .iter()
            .enumerate()
            .filter_map(|(idx, turn)| turn_counts_as_user_turn(turn).then_some(idx))
            .collect::<Vec<_>>();
        let cut_idx = if n == 0 {
            self.turns.len()
        } else if n >= user_turn_positions.len() {
            0
        } else {
            user_turn_positions[user_turn_positions.len() - n]
        };
        let removed_turn_ids = if cut_idx < self.turns.len() {
            self.turns[cut_idx..]
                .iter()
                .map(|turn| turn.id.clone())
                .collect()
        ...
        if cut_idx == 0 {
            self.turns.clear();
        } else if cut_idx < self.turns.len() {
...
fn turn_counts_as_user_turn(turn: &Turn) -> bool {
    turn.items
        .iter()
...
mod tests {
    ...
    #[test]
    fn thread_rollback_after_compaction_drops_latest_user_turn_and_compaction() {
        let items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(TurnStartedEvent {
                turn_id: "turn-1".into(),
                trace_id: None,
