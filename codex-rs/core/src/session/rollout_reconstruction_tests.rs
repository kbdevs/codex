...
#[tokio::test]
async fn reconstruct_history_rollback_counts_typed_inter_agent_metadata_turns() {
    let (session, turn_context) = make_session_and_context().await;
    let first_context_item = turn_context.to_turn_context_item();
    let first_turn_id = first_context_item
        .turn_id
        .clone()
        .expect("turn context should have turn_id");
    let assistant_turn_id = "typed-assistant-instruction-turn".to_string();
    let assistant_turn_context = TurnContextItem {
        turn_id: Some(assistant_turn_id.clone()),
        ..first_context_item.clone()
    };
...
#[tokio::test]
async fn reconstruct_history_rollback_after_compaction_drops_latest_user_turn_and_compaction() {
    let (session, turn_context) = make_session_and_context().await;
    let first_context_item = turn_context.to_turn_context_item();
    let first_turn_id = first_context_item
        .turn_id
        .clone()
        .expect("turn context should have turn_id");
    let second_turn_id = "turn-before-compaction".to_string();
    let compact_turn_id = "compact-after-latest-user".to_string();
    let turn_one_user = user_message("turn 1 user");
    let turn_one_assistant = assistant_message("turn 1 assistant");
    let turn_two_user = user_message("turn 2 user");
    let turn_two_assistant = assistant_message("turn 2 assistant");
    let compacted_history = vec![
        turn_one_user.clone(),
        assistant_message("summary mentions turn 2"),
    ];
    ...
    let rollout_items = vec![
        ...
        RolloutItem::Compacted(CompactedItem {
            message: "summary mentions turn 2".to_string(),
...
#[tokio::test]
async fn reconstruct_history_rollback_after_compaction_trims_compacted_history() {
    let (session, turn_context) = make_session_and_context().await;
    let turn_one_user = user_message("turn 1 user");
    let turn_one_assistant = assistant_message("turn 1 assistant");
    let turn_two_user = user_message("turn 2 user");
    let turn_two_assistant = assistant_message("turn 2 assistant");
    let compact_turn_id = "compact-truncated-rollout".to_string();
...
#[tokio::test]
async fn reconstruct_rollback_counts_raw_turns_before_trimming_compaction() {
    let (session, turn_context) = make_session_and_context().await;
    let turn_one_user = user_message("turn 1 user");
    let turn_one_assistant = assistant_message("turn 1 assistant");
    let turn_two_user = user_message("turn 2 user");
    ...
    let compact_turn_id = "compact-before-raw-turn".to_string();
    let raw_turn_id = "raw-turn-after-compaction".to_string();
    ...
    let rollout_items = vec![
        ...
        RolloutItem::ResponseItem(turn_three_user),
        RolloutItem::ResponseItem(turn_three_assistant),
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            codex_protocol::protocol::TurnCompleteEvent {
                turn_id: raw_turn_id,
                last_agent_message: None,
